...
fn main() {
    ...
    {
        tunnel::recover_orphaned_takeover();
    }
    ...
    tauri::Builder::default()
        ...
        .expect("error while building chatgpt-shell")
        .run(|_app_handle, event| {
            // 退场必恢复：正常退出（Cmd+Q/关窗）时精确还原用户网络
            match event {
                tauri::RunEvent::ExitRequested { .. } => {
                    tunnel::log_line("[exit-req] 应用请求退出，释放代理");
                    tunnel::release_proxy();
                }
                tauri::RunEvent::Exit => {
                    tunnel::log_line("[exit] 应用退出，释放代理");
                    tunnel::release_proxy();
                }
                _ => {}
            }
