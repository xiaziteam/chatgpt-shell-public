...
static SAVED_SOCKS_PROXY: Mutex<Option<(bool, String, u16)>> = Mutex::new(None);

#[cfg(target_os = "macos")]
static SAVED_MAC_PAC: Mutex<Option<Vec<(String, bool, String)>>> = Mutex::new(None);
...
#[cfg(target_os = "windows")]
static SAVED_WIN_PROXY: Mutex<Option<(String, String, String)>> = Mutex::new(None);
...
pub fn set_system_proxy(enable: bool, socks_port: u16) {
    ...
    {
        if enable {
            let cur_enable = win_reg_query("ProxyEnable").unwrap_or_else(|| "0x0".to_string());
            let cur_server = win_reg_query("ProxyServer").unwrap_or_default();
            let cur_override = win_reg_query("ProxyOverride").unwrap_or_default();
            *SAVED_WIN_PROXY.lock().unwrap() = Some((cur_enable, cur_server, cur_override));
            win_reg_set("ProxyEnable", "1", "REG_DWORD");
            win_reg_set("ProxyServer", &format!("socks=127.0.0.1:{}", socks_port), "REG_SZ");
            win_reg_set("ProxyOverride", "<local>", "REG_SZ");
            log_line(&format!("[proxy] Windows系统代理已开 socks=127.0.0.1:{}", socks_port));
            win_inet_refresh();
        } else if let Some((e, srv, ovr)) = SAVED_WIN_PROXY.lock().unwrap().take() {
            win_reg_set("ProxyEnable", if e == "0x1" { "1" } else { "0" }, "REG_DWORD");
            if srv.is_empty() { win_reg_del("ProxyServer"); } else { win_reg_set("ProxyServer", &srv, "REG_SZ"); }
            if ovr.is_empty() { win_reg_del("ProxyOverride"); } else { win_reg_set("ProxyOverride", &ovr, "REG_SZ"); }
            log_line("[proxy] Windows系统代理已恢复旧值");
            win_inet_refresh();
        } else {
            win_reg_set("ProxyEnable", "0", "REG_DWORD");
            log_line("[proxy] Windows系统代理已关");
            win_inet_refresh();
        }
    }
    #[cfg(target_os = "macos")]
    {
        ...
        if enable {
            // 保存当前每个service的PAC状态
            let mut saved_pac_states: Vec<(String, bool, String)> = Vec::new();
            for svc in &services {
                let enabled = get_autoproxy_state(svc);
                let url = get_autoproxy_url(svc);
                saved_pac_states.push((svc.clone(), enabled, url));
            }
            *SAVED_MAC_PAC.lock().unwrap() = Some(saved_pac_states);
            ...
            log_line(&format!("[proxy] macOS PAC代理已开 url={} services={}个", DAEMON_PAC_URL, services.len()));
        } else {
            // 恢复所有service的PAC状态
            let saved = SAVED_MAC_PAC.lock().unwrap().take();
            if let Some(states) = saved {
                for (svc, was_enabled, old_url) in &states {
                    let _ = std::process::Command::new("networksetup")
                        .args(["-setautoproxystate", svc, "off"])
                        .output();
                    if *was_enabled && !old_url.is_empty() {
                        let _ = std::process::Command::new("networksetup")
                            .args(["-setautoproxyurl", svc, old_url])
                            .output();
                        let _ = std::process::Command::new("networksetup")
                            .args(["-setautoproxystate", svc, "on"])
                            .output();
                ...
                // 无保存状态时，对所有service关PAC
                for svc in &services {
                    let _ = std::process::Command::new("networksetup")
                        .args(["-setautoproxystate", svc, "off"])
                        .output();
                    let _ = std::process::Command::new("networksetup")
                        .args(["-setautoproxyurl", svc, ""])
                        .output();
                }
            }
            log_line("[proxy] macOS PAC代理已关");
        }
    }
    #[cfg(target_os = "linux")]
    {
        if enable {
            ...
            let _ = std::process::Command::new("gsettings")
                .args(["set", "org.gnome.system.proxy", "mode", "none"])
                .output();
            log_line("[proxy] Linux GNOME SOCKS代理已关");
        }
    }
}
...
fn get_network_service() -> String {
    ...
    match output {
        ...
        Err(e) => log_line(&format!("[proxy] networksetup failed: {}", e)),
    }
    String::new()
}
...
fn get_autoproxy_state(service: &str) -> bool {
    let output = std::process::Command::new("networksetup")
        .args(["-getautoproxystate", service])
        .output();
    match output {
        Ok(out) => {
            let text = String::from_utf8_lossy(&out.stdout);
